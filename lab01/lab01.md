...
2. Какие языки (если есть) ваш браузер может принимать? В захваченном сеансе какую еще
   ...
   - ru-RU,ru;q=0.9,en-US;q=0.8,en;q=0.7,bg;q=0.6
   - Время запроса, версия сервера, тип соединения, информация о моем браузере
...
2. Проверьте содержимое ответа сервера. Вернул ли сервер содержимое файла явно? Как вы
   это можете увидеть?
   - Да
   - Ммеется поле File Data/сам документ
...
4. Какой код состояния HTTP и фраза возвращаются сервером в ответ на этот второй запрос
   ...
   - 304
   - Нет, не вернул. Содержимое можно получить по etag.
...
2. Можете ли вы сказать, загрузил ли ваш браузер два изображения последовательно или
   ...
   - Если смотреть на временные метки GET-запросов и их завершения, то они не пересекаются. Однако *нельзя* сделать вывод, что загрузка непараллельная.
