...
2. Почему ICMP-пакет не обладает номерами исходного и конечного портов?
   - Так как ICMP не использует порты, он не является транспортным протоколом.
3. Рассмотрите один из ping-запросов, отправленных вашим хостом. Каковы ICMP-тип и кодовый
   ...
   - 2 байта контрольной суммы, 2 байта порядкового номера и 2 байта идентификатора
...
1. Рассмотрите ICMP-пакет с эхо-запросом на вашем скриншоте. Отличается ли он от ICMP-пакетов
   ...
   - Он работает поверх UDP, а не ICMP. Он также использует разные типы и коды.
2. Рассмотрите на вашем скриншоте ICMP-пакет с сообщением об ошибке. В нем больше полей,
   ...
   - В дополнительных полях содержится информация о типе ошибки, IP-адресе отправителя и 
   оригинальном заголовке пакета, вызвавшего ошибку.
3. Рассмотрите три последних ICMP-пакета, полученных исходным хостом. Чем эти пакеты
   ...
   - В них содержится дополнительная информация об destination port внутри UDP-заголовка.
   - Да у меня все полученные пакеты это пакеты с ошибками..
4. Есть ли такой канал, задержка в котором существенно превышает среднее значение? Можете
   ...
   - Межконтинентальный канал
   - Да, можно определить местоположение маршрутизаторов по их именам и IP-адресам.
