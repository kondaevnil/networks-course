...
4. Какой порядковый номер у SYNACK-сегмента, отправленного сервером gaia.cs.umass.edu
   4. Какой порядковый номер у SYNACK-сегмента, отправленного сервером gaia.cs.umass.edu на компьютер клиента в ответ на SYN-сегмент? Какое значение хранится в поле подтверждения в SYNACK-сегменте? Как сервер gaia.cs.umass.edu определил это значение? Как определяется, что это именно SYNACK-сегмент?
      - Порядковый номер: 0
      - Значение в поле подтверждения: порядковый номер SYN-сегмента клиента + 1
      - Сервер определяет это значение, используя порядковый номер, указанный в SYN-сегменте клиента, и увеличивает его на единицу.
      - Это SYNACK-сегмент, так как в нем установлены флаги SYN и ACK.
...
6. Рассмотрите TCP-сегмент, содержащий команду POST протокола HTTP, как первый TCP-сегмент 
   ...
   - 1, 649, 2097, 3545, 4993, 6441, ...
   - через 0.131235000 после отправки первого фрейма tcp стрима, с задержкой друг между другом примерно 0.000001000 секунд
   - Первый ACK через 0.264092000 после отправки первого фрейма tcp стрима
   - 0.264092000 - 0.131235000 = 0.132857000 = RTT, для первого сегмента
7. Чему равна пропускная способность (количество байтов, передаваемых в единицу
   ...
   - MSS / RTT = (1460 байт) / (0.132857000) = 10960.000 байт/с
   - необходимо просуммировать размеры всех TCP-сегментов, которые были отправлены в течение времени, когда они были отправлены.
   - Пропускная способность = (размер всех TCP-сегментов) / (время, за которое они были отправлены)
