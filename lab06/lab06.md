...
2. Определите длину (в байтах) для каждого поля UDP-заголовка, обращаясь к отображаемой
   ...
   - length - 2 байта
   - checksum - 2 байта
   - payload - в моем случае 28 байт
3. Значение в поле Length (Длина) – это длина чего?
   - длина заголовка + payload в байтах
4. Какое максимальное количество байт может быть включено в полезную нагрузку UDP-пакета?
   - 8 байт уходит на заголовок, значит payload может быть 65528 байт, однако при использовании ipv4 необходимо еще 20 байт на ip заголовок, поэтому 65507 байт
...
7. Проверьте UDP-пакет и ответный UDP-пакет, отправляемый вашим хостом. Определите
   ...
   - порт моего маршрутизатора при DNS запросах - 53, порт моего хоста в отправляемом и ответном UPD остается один и тот же 14526
